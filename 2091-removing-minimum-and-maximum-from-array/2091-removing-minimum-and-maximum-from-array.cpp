...
public:
    int minimumDeletions(vector<int>& nums) {
        ...
        for(int index =1; index < numsSize; index++){
           ...
           else if(nums[index]< nums[minElementIndex])
                minElementIndex = index;
        }
        int minIndex = maxElementIndex < minElementIndex ? maxElementIndex:minElementIndex ;
        int maxIndex = maxElementIndex > minElementIndex ? maxElementIndex:minElementIndex ;
